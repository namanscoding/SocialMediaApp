...
public class DiscussionService {
    ...
    public List<Discussions> getDiscussionsByTags(List<String> hashTags) {
        List<Discussions> discussions = discussionRepository.findByHashTagsIn(hashTags);
        if (!discussions.isEmpty()) {
            for (Discussions discussion : discussions) {
                discussion.setViewCounts(discussion.getViewCounts() + 1);
    ...
    public List<Discussions> getDiscussionsByText(String text) {
        List<Discussions> discussions = discussionRepository.findByTextContainingIgnoreCase(text);
        if (!discussions.isEmpty()) {
            for (Discussions discussion : discussions) {
                discussion.setViewCounts(discussion.getViewCounts() + 1);
                discussionRepository.save(discussion);
    ...
    public int getViewCounts(String discussionId){
        Discussions discussion = discussionRepository.findById(discussionId).orElse(null);
        int viewCounts = 0;
        if (discussion != null) {
            viewCounts = discussion.getViewCounts();
        }
        return viewCounts;
    }
}
